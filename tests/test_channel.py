...
async def test_notify_sends_plain_payload_and_writes_nothing(http_script, fake_redis):
    # An ok:true body WITHOUT ts suffices — notify needs no ts, stores no
    # correlation, and the text is the bare message (no deadline, no
    # reply-in-thread instruction, no callback link).
    http_script.results.append(_ok_response(ts=None))

    await SlackChannel().notify(ChannelNotification(message="Deploy finished."))
