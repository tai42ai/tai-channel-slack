...
``tai42_channel_slack`` is on the allowlist. The rule (see the README): the shipped
package imports
...
The runtime walk imports ``tai42_channel_slack`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_channel_slack`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_channel_slack`` graph pulls in -- the
