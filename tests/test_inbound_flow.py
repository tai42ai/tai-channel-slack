...
async def test_no_recipients_configured_raises_and_releases_claim(fake_redis, monkeypatch):
    # No default recipient and an empty allowlist: no conversation could ever
    # have received a question — operator misconfiguration, raised loudly.
    monkeypatch.delenv("CHANNEL_SLACK_DEFAULT_RECIPIENT")
    monkeypatch.delenv("CHANNEL_SLACK_ALLOWED_RECIPIENTS")
    reset_all_settings()

    with pytest.raises(ValueError, match="CHANNEL_SLACK_DEFAULT_RECIPIENT or CHANNEL_SLACK_ALLOWED_RECIPIENTS"):
        await slack_inbound(_signed(_event_body(event=_reply_event())))

    assert _DEDUPE_KEY not in fake_redis.store  # released so a retry reprocesses
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(_reply_event(bot_id="B0BOT"), id="bot-echo"),
        pytest.param(_reply_event(channel="C0OTHER"), id="wrong-channel"),
        pytest.param(_reply_event(thread_ts=None), id="top-level-message"),
        pytest.param(_reply_event(type="reaction_added"), id="not-a-message"),
    ],
)
async def test_non_answer_traffic_is_acked_ignored(fake_redis, http_script, event):
    _seed_correlation(fake_redis)
...
async def test_thread_with_no_pending_question_is_ignored(fake_redis, http_script):
    # No correlation entry seeded: chatter in a thread that was never ours (or
