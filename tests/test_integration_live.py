...
``CHANNEL_SLACK_BOT_TOKEN`` / ``CHANNEL_SLACK_DEFAULT_RECIPIENT`` /
``CHANNEL_SLACK_REDIS_URL`` environment; the suite skips cleanly when any of
them is unset.

Outbound only: the reply leg is human-by-definition, so it is proven by the
offline flow tests, not here — and nothing polls ``conversations.history``
(tightly rate-limited for new non-Marketplace apps). The posted message stays
visible in the channel; its text labels it as an integration-test question.
"""
